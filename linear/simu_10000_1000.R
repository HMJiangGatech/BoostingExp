...
if( useSimData)
{
  ...
  test_gausnet(sim_data,alg="asp-newton",trialN = trialN,prec=1.5*1e-8,ratio=ratio, nlambda = nlambda)
  test_gausnet(sim_data,alg="greedy",    trialN = trialN,prec=8*1e-5,ratio=ratio, nlambda = nlambda)
  test_gausnet(sim_data,alg="cyclic",    trialN = trialN,prec=1*1e-5,ratio=ratio, nlambda = nlambda)
  test_gausnet(sim_data,alg="glmnet",    trialN = trialN,prec=1*1e-8,ratio=ratio, nlambda = nlambda)
  test_gausnet(sim_data,alg="gcdnet",    trialN = trialN,prec=3*1e-6,ratio=ratio, nlambda = nlambda)
  test_gausnet(sim_data,alg="ncvreg",    trialN = trialN,prec=3*1e-5,ratio=ratio, nlambda = nlambda)
