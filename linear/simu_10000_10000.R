...
if( useSimData)
{
  ...
  test_gausnet(sim_data,alg="greedy",    trialN = trialN,prec=1*1e-5,ratio=ratio, nlambda = nlambda)
  ...
  test_gausnet(sim_data,alg="ncvreg",    trialN = trialN,prec=3*1e-6,ratio=ratio, nlambda = nlambda)
  test_gausnet(sim_data,alg="fista",     trialN = 1,prec=1*1e-5,ratio=ratio, nlambda = nlambda,fista_it = 100)
  rm(sim_data)
}
