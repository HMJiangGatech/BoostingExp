...
// [[Rcpp::export]]
double compute_sqrtlasso_loss(NumericMatrix X, NumericVector y, 
                            NumericVector Xbeta, NumericVector beta, 
                            double a0, double lambda) {
  int n = X.nrow();
  int d = X.ncol();
  ...
  double L = 0.0;
  for (int i = 0; i < n; i++){
    L +=  (y[i] - Xbeta[i] - a0) * (y[i] - Xbeta[i] - a0);
  }
  L = L/n;
  L = pow(L,0.5);
  Rcout << L<<' ';
...
// [[Rcpp::export]]
double compute_gausnet_loss(NumericMatrix X, NumericVector y, 
                           NumericVector Xbeta, NumericVector beta, 
                           double a0, double lambda) {
  int n = X.nrow();
  int d = X.ncol();
